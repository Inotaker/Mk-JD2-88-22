...
import by.it.academy.Mk_JD2_88_22.homework.hw1.storage.hibernate.entity.AuditUserEntity;
...
public class HibernateDBAuditStorage {

    public static ArrayList<AuditUser> select() {

        ArrayList<AuditUser> audit = new ArrayList<>();
        ...
        CriteriaQuery<AuditUserEntity> query = builder.createQuery(AuditUserEntity.class);
        Root<AuditUserEntity> from = query.from(AuditUserEntity.class);
        query.select(from);

        List<AuditUserEntity> resultList = manager.createQuery(query).getResultList();
        for (AuditUserEntity auditUserEntity : resultList) {
            AuditUser addedAudit = new AuditUser(auditUserEntity.getId(), auditUserEntity.getDtCreate(), auditUserEntity.getText(), auditUserEntity.getUser(), auditUserEntity.getAuthor());
            audit.add(addedAudit);
        }
        return audit;
    }
